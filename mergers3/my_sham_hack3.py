...
class SMFClass:
    ...
    def __init__(self, source='li-march', redshift=0.1, scat=0, hubble=0.7,
        ...
        if source == 'li':
            ...
            self.slopes=np.array([-1.89,-1.74,-2.,-1.89])
            self.make_splines()
            self.initialize_redshift(redshift)
        elif source=='caputi':
            '''
            ...
            self.redshifts=np.array([0.6,0.8,1.2,1.6,2.15,3.,4.,4.5])
            self.mchars=np.array([11.67,11.67,11.61,11.75,11.82,11.3,11.3,11.3])
            self.amplitudes=10.**np.array([-3.7,-3.7,-3.47,-3.85,
                                           -4.17,-3.94,-4.12,-4.12])
            self.slopes=np.array([-1.44,-1.44,-1.47,-1.6,-1.84,
                                  -1.86,-1.8,-1.8])
            self.make_splines()
            self.initialize_redshift(redshift)
        elif source == 'baldry':
    ...
    def make_numden_m_spline(self, redshift=0.1, scat=0):
        ...
        #print numdens
        #print numdens[1:]-numdens[:-1]
        #print np.where((numdens[1:]-numdens[:-1])>0)

        self.m_log_numden_spl = interpolate.splrep(log10(numdens)[::-1], 
                                                   m_stars[::-1])
        ...
        #Trying an error workaround -PS########################################
            ...
            while len(bad)>0:
                i+=1
                numdens[bad+1]=numdens[bad]-1e-14
                dnumden=numdens[1:]-numdens[:-1]
                #print numdens
                #print numdens[1:]-numdens[:-1]
                #print np.where((numdens[1:]-numdens[:-1])>0)
        ...
        if scat:
            ...
            for mi in range(m_stars.size):
                #Evaluate the definite integral of dn/dlogm from m_stars[mi] to
                #the maximum m_star i.e. the cumulative number density
                numdens_scat[mi] = interpolate.splint(m_stars[mi], 
                                                      m_stars.max(),
                                                      self.dndm_m_scat_spl)
                #numdens_scat[mi] += 1e-20 * (1 - mi * 0.001)
